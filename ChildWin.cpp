...
if (MainForm->grayTable){
        if(GrayLevelsTable->pChild==this){
        GrayLevelsTable->pChild=NULL;
        }
}
...
//---------------------------------------------------------------------------
void __fastcall TMDIChild::ImageMouseMove(TObject *Sender, TShiftState Shift, int X, int Y)
{
PixelXMove=X;
PixelYMove=Y;
...
if (MainForm->grayTable){
        GrayLevelsTable->pChild=this;
        GrayLevelsTable->NuevaTabla();
}
